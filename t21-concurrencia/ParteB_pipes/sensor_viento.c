//t21-concurrencia/ParteB_pipes/sensor_viento.c
...
int main(void){
    const char* fifo = "/tmp/fifo_viento";
    const char* sname = "VIENTO";
    int base = 5;   // segundos base
    int jitter = 2; // +/-
    ...
    fprintf(stderr, "[VIENTO] Abriendo FIFO %s ...\\n", fifo);
    int fd = open(fifo, O_WRONLY);
    if(fd<0){ perror("open"); return 1; }

    fprintf(stderr, "[VIENTO] Iniciando generacion indeterminada (base=%d, jitter=+/- %d)...\\n", base, jitter);
    char line[128], ts[64];
    while(1){
